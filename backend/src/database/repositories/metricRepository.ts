...
export const imcrementDeckReview = async (user_id: string): Promise<void> => {
	try {
		...
		const decks_reviews =
            userMetrics[userMetrics.length - 1].getDataValue("decks_reviews");
		const reviews =
            userMetrics[userMetrics.length - 1].getDataValue("reviews");
...
export const imcrementSummariesReview = async (
	user_id: string,
): Promise<void> => {
	try {
		const userMetrics = await MetricsModel.findAll({
