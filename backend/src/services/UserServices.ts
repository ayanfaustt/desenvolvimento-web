...
class UserServices{
    ...
    async createUser (username: string, email: string, password: string): Promise<void> {
        ...
        const isAlreadyExist = await UserRepository.getOnlyUserByUsername(username);
    
        if (!isAlreadyExist) throw new Error("User already exist !");
        
        const hash = await this.hashPassword(password);
        const isUserCreated = await UserRepository.createUser(username, email, hash);
        
        if (!isUserCreated) throw new Error("User not created !");
        
        const date = new Date();
        await createMetricsRepository(isUserCreated.getDataValue('id'), 0, 0, 0, date.toDateString());
    ...
    async updateUser (username: string, email: string, password: string): Promise<void> {
        ...
        try {
            ...
            const hash = await this.hashPassword(password);
            const isUserUpdated = UserRepository.createUser(username, email, hash);
    ...
    getAllUserInfoByUserName = async (username: string): Promise<Model> => {
       try {
            const userInfo = await UserRepository.getAllUserInfoByUserName(username);
    ...
    async getAllUserInfoByUserEmail (email: string): Promise<Model> {
       try {
            const userInfo = await UserRepository.getAllUserInfoByUserEmail(email); 
